...
export default function InfrastructureEcosystem({ services }: { services: Service[] }) {
  ...
  const getIcon = (title: string) => {
    ...
    if (t.includes('solar') || t.includes('retail')) return <Zap className="h-8 w-8" />;
    if (t.includes('automation') || t.includes('educational') || t.includes('campus')) return <Cpu className="h-8 w-8" />;
    if (t.includes('network') || t.includes('switch') || t.includes('industrial') || t.includes('psu')) return <Server className="h-8 w-8" />;
